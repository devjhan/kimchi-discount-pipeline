...
# ============================================================
#
# 1. launchctl bootout (모든 com.kimchidiscountpipeline.* agent 제거; 이전 v2/v3 잔재도 정리)
# 2. ~/Library/LaunchAgents/com.investment_v[23].*.plist 및 com.kimchidiscountpipeline.*.plist 삭제
...
removed=0
# 마이그레이션 tolerant glob — 신 kimchidiscountpipeline + 구 v2/v3 잔재 동시 정리 (X-1, _labels.py 동기).
for plist in "$HOME/Library/LaunchAgents/com.investment_v"[23]"."*.plist "$HOME/Library/LaunchAgents/com.kimchidiscountpipeline."*.plist; do
  [ -f "$plist" ] || continue
...
remaining="$(ls "$HOME/Library/LaunchAgents/com.investment_v"[23]"."*.plist "$HOME/Library/LaunchAgents/com.kimchidiscountpipeline."*.plist 2>/dev/null | wc -l | tr -d ' ')"
if [ "$remaining" -ne 0 ]; then
  echo "[uninstall] WARN: $remaining plist 잔존" >&2
fi
launchctl print "gui/$UID_NUM/com.kimchidiscountpipeline.daily_pipeline" 2>&1 | head -3 || true
