...
``$AUDIT_DIR/shadow-portfolio/trade-log-{tier}.csv`` append-only. columns:
...
def append_closed_trades(trades: list[ClosedTrade]) -> list[Path]:
    """tier 별로 그룹화해 shadow-portfolio/trade-log-{tier}.csv 에 append. 기록된 파일 경로 list 반환."""
    shadow_dir = _boundary.resolve_path("shadow_dir")
    ...
    for tier_key, tier_trades in by_tier.items():
        path = shadow_dir / f"trade-log-{tier_key}.csv"
