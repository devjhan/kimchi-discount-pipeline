...
`governance/schedules.yaml`. OS 별
artifact 는 `infrastructure/scheduling/*_generator.py`
가 derived 결과로 emit 한다.
...
활성화 가능. 이때 `infrastructure/scheduling/install.sh`
의 OS 분기에 cloud target 옵션 추가를 권장.
