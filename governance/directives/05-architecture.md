...
domains/{bc}/      = BC 내부 HOW (코드와 함께 변함). .guidelines/ 디렉토리는 ADR-0011에 의해 skills로 흡수·삭제됨.
```
...
**Hook** (ADR-0010으로 파기: `block_anti_patterns.sh`): → 대체: **fitness test** `tests/architecture/test_dependency_direction.py` · `test_boundary_gate.py`
...
**Hook**: inject_only — HOW 템플릿은 `governance/decisions/0005-boundary-ports-and-adapters.md` 참조 (`.guidelines/` 삭제로 ADR 직접 인용). 불변식 C/D 는 `test_boundary_gate.py` 가 검증.
...
(.guidelines/ 디렉토리는 ADR-0011에 의해 skills로 흡수·삭제됨)
```

**Hook**: **fitness test** `tests/architecture/test_doc_completeness.py` 가 ADR 인덱스 무결성 검증 (`.guidelines/` 검사는 ADR-0011로 제거).
