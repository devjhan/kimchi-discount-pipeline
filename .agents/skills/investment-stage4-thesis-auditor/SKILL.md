...
본 skill은 `Recommended Default ≠ Decision` 원칙을 따르며, `Needs Input`
상황에서는 묶음 질문으로 사용자에게 escalate한다. 코드 도메인 룰 (BC,
aggregate, Code-Grounded Verification 등) 은 그대로 직역하지 않는다.
...
본 skill 산출 후 다음을 사용자에게 manual step으로 권고:
