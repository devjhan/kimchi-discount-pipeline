...
| C4 | 외부 신호 intake | 뉴스/트윗/블로그/리포트 → fact-only paraphrase + redaction. thesis 즉시 변경 0 | 명시적 `/ingest-external-signal` 명령 | `ingest-external-signal` skill | `config/signals/{ticker}/{date}-{seq}.md` |
