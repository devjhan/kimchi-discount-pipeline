...
2. `common/profile-engine.md` — PolicyEngine Protocol (ports/llm.py), EnrichCutoffProfile schema (ADR-0006 fat contract), ProfileRegistry API, governance/profiles/ 포맷
...
- Cutover 오해: `--use-profile-registry` 기본 OFF (F-21 open), `governance/profiles/` 빈 상태 → universe/screener 는 default 경로로 동작 중
...
- `governance/profiles/{ticker}/` — phase 3 committed profile (결정론 gate 통과 후)
