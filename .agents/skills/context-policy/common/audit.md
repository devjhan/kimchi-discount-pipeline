...
`ViolationLog` 는 `_shared.audit.log.ViolationLog` 의 thin subclass (`bc_name="policy"`). JSONL: `$AUDIT_DIR/violations/policy/{date}.jsonl`.
...
| G10 | 외부 신호는 `/ingest-external-signal` 만 | `Trigger` 는 `payload_ref` 만; evidence 는 `telemetry/external_signals/` |
