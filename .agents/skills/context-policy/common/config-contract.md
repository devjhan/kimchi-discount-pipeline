...
| `SCHEMA_VERSION` | `_shared/profile_registry/schema.py` | `"policy-profile-v1"` | profile schema gate (on-disk serde 단일 권위 = `_shared/policy_profile/`) |
| `DART_API_KEY` | `.env` | (secret) | DART intake auth |

## Committed artifact schema

policy 가 소유하는 유일한 YAML "config" (= `governance/policy/profiles/{ticker_dir}/v{N}.yaml`):

| on-disk key | dataclass field | 비고 |
|---|---|---|
| `schema` | `schema_version` | `policy-profile-v1` 고정 (scope-tagged 통합 스키마; per-ticker 파일은 scope=ticker, `EnrichCutoffProfile` 은 그 in-memory view) |
| `version` | `profile_version` | ticker 별 monotonic int ≥ 1 |
| `description` | `description` | D-CFG-1 헤더 미러 |
| `scope` | — | per-ticker 파일은 `ticker` 고정 |
| `key` | `ticker` | `KR:NNNNNN` (통합 스키마 `key`; profile_registry serde 가 `ticker` 로 투영) |
